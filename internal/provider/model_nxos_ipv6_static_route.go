...
func (data *IPv6StaticRoute) updateFromBody(res gjson.Result) {
	...
	for c := range data.NextHops {
		...
		if !data.NextHops[c].InterfaceId.IsNull() {
			data.NextHops[c].InterfaceId = types.StringValue(ripv6Nexthop.Get("ipv6Nexthop.attributes.nhIf").String())
		} else {
			data.NextHops[c].InterfaceId = types.StringNull()
		}
		if !data.NextHops[c].Address.IsNull() {
			data.NextHops[c].Address = types.StringValue(ripv6Nexthop.Get("ipv6Nexthop.attributes.nhAddr").String())
		} else {
			data.NextHops[c].Address = types.StringNull()
		}
		if !data.NextHops[c].VrfName.IsNull() {
			data.NextHops[c].VrfName = types.StringValue(ripv6Nexthop.Get("ipv6Nexthop.attributes.nhVrf").String())
		} else {
			data.NextHops[c].VrfName = types.StringNull()
		}
		if !data.NextHops[c].Description.IsNull() {
			data.NextHops[c].Description = types.StringValue(ripv6Nexthop.Get("ipv6Nexthop.attributes.descr").String())
		} else {
			data.NextHops[c].Description = types.StringNull()
		}
		if !data.NextHops[c].Object.IsNull() {
			data.NextHops[c].Object = types.Int64Value(ripv6Nexthop.Get("ipv6Nexthop.attributes.object").Int())
		} else {
			data.NextHops[c].Object = types.Int64Null()
		}
		if !data.NextHops[c].Preference.IsNull() {
			data.NextHops[c].Preference = types.Int64Value(ripv6Nexthop.Get("ipv6Nexthop.attributes.pref").Int())
		} else {
			data.NextHops[c].Preference = types.Int64Null()
		}
		if !data.NextHops[c].Tag.IsNull() {
			data.NextHops[c].Tag = types.Int64Value(ripv6Nexthop.Get("ipv6Nexthop.attributes.tag").Int())
		} else {
