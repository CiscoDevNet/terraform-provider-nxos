// Code generated by "gen/generator.go"; DO NOT EDIT.

package provider

import (
	"fmt"

	"github.com/hashicorp/terraform-plugin-framework/types"
	"github.com/netascode/go-nxos"
	"github.com/tidwall/gjson"
)

type BGPInstanceDomainPeerContainerAf struct {
	Device     types.String `tfsdk:"device"`
	Dn         types.String `tfsdk:"id"`
	Vrf_name   types.String `tfsdk:"vrf"`
	Name       types.String `tfsdk:"template_name"`
	Type       types.String `tfsdk:"af"`
	Ctrl       types.String `tfsdk:"control"`
	SendComExt types.String `tfsdk:"send_community_extended"`
	SendComStd types.String `tfsdk:"send_community_standard"`
}

func (data BGPInstanceDomainPeerContainerAf) getDn() string {
	return fmt.Sprintf("sys/bgp/inst/dom-[%s]/peercont-[%s]/af-[%s]", data.Vrf_name.Value, data.Name.Value, data.Type.Value)
}

func (data BGPInstanceDomainPeerContainerAf) getClassName() string {
	return "bgpPeerAf"
}

func (data BGPInstanceDomainPeerContainerAf) toBody() nxos.Body {
	attrs := nxos.Body{}.
		Set("type", data.Type.Value).
		Set("ctrl", data.Ctrl.Value).
		Set("sendComExt", data.SendComExt.Value).
		Set("sendComStd", data.SendComStd.Value)
	return nxos.Body{}.SetRaw(data.getClassName()+".attributes", attrs.Str)
}

func (data *BGPInstanceDomainPeerContainerAf) fromBody(res gjson.Result) {
	data.Dn.Value = res.Get("*.attributes.dn").String()
	data.Type.Value = res.Get("*.attributes.type").String()
	data.Ctrl.Value = res.Get("*.attributes.ctrl").String()
	data.SendComExt.Value = res.Get("*.attributes.sendComExt").String()
	data.SendComStd.Value = res.Get("*.attributes.sendComStd").String()
}

func (data *BGPInstanceDomainPeerContainerAf) fromPlan(plan BGPInstanceDomainPeerContainerAf) {
	data.Device = plan.Device
	data.Vrf_name.Value = plan.Vrf_name.Value
	data.Name.Value = plan.Name.Value
}
