...
func (data Feature) toBody(config Feature) nxos.Body {
	...
	childrenPath := data.getClassName() + ".children"
	{
		attrs = "{}"
		childBody := ""
		childBody, _ = sjson.SetRaw(childBody, "fmEntity.attributes", attrs)
		parentAttrs := attrs
		parentPath := childrenPath
		nestedChildrenPath := "fmEntity.children"
		...
		childBody = body
		body = prevBody
		if parentAttrs != "{}" || gjson.Get(childBody, "fmEntity.children").Exists() {
...
func (data Feature) toBodyWithDeletes(ctx context.Context, state Feature, config Feature, importing bool) nxos.Body {
	...
	if !importing {
		...
		for key := range state.FeatureSets {
			if configChild, ok := config.FeatureSets[key]; ok {
				stateChild := state.FeatureSets[key]
				_ = stateChild
				_ = configChild
				for mi, mv := range gjson.Get(body.Str, bodyPath).Array() {
					if mv.Get("fsetFeatureSet.attributes.name").String() == key {
						if !stateChild.AdminState.IsNull() && configChild.AdminState.IsNull() {
							body.Str, _ = sjson.Set(body.Str, bodyPath+"."+strconv.Itoa(mi)+".fsetFeatureSet.attributes."+"adminSt", "DME_UNSET_PROPERTY_MARKER")
						}
						break
					}
				}
