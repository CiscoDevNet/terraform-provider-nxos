...
page_title: "nxos_ipv6_access_list Resource - terraform-provider-nxos"
subcategory: "Security"
description: |-
  This resource can manage IPv6 access control lists (ACLs) on NX-OS devices, including ACL entries with match criteria such as source/destination IPv6 prefixes, ports, protocols, and TCP flags. It also supports applying ACLs to interfaces in ingress or egress direction.
  API Documentation
...
# nxos_ipv6_access_list (Resource)
...
- `icmp_string` (String) ICMPv6 type.
  - Choices: `unreachable`, `no-route`, `no-admin`, `beyond-scope`, `destination-unreachable`, `port-unreachable`, `packet-too-big`, `time-exceeded`, `hop-limit`, `reassembly-timeout`, `parameter-problem`, `header`, `next-header`, `parameter-option`, `echo-request`, `echo-reply`, `mld-query`, `mld-report`, `mld-reduction`, `router-solicitation`, `router-advertisement`, `nd-ns`, `nd-na`, `redirect`, `router-renumbering`, `renum-command`, `renum-result`, `renum-seq-number`, `mldv2`
...
- `protocol` (String) Protocol for access-list entry.
  - Choices: `ipv6`, `tcp`, `udp`, `gre`, `esp`, `ahp`, `icmp`, `eigrp`, `ospf`, `nos`, `pim`, `pcp`, `sctp`, `udf`
...
```shell
terraform import nxos_ipv6_access_list.example ""
```
